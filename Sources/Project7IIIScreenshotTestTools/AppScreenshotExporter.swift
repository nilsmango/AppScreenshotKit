...
/// Exports screenshots using Project7IIIScreenshots and saves them to disk or attaches them to XCTest.
///
...
public class AppScreenshotExporter {
    ...
    private static func resolveDefaultBezelURL() -> URL {
        ...
        if (try? FileManager.default.subpathsOfDirectory(atPath: bundleURL.path))?.isEmpty == false {
            return bundleURL
        }
        let cachesDirectory = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
        return cachesDirectory.appending(
            path: "com.nilsmango.Project7IIIScreenshots/AppleDesignResource/Bezels"
        )
    }
